...
import { getallAppoinmentForAdmin } from "@/type/type";
import { getAppoinments } from "@/apis/admin/getallappointments";

const AdminAppoints = () => {
  const [appointments, setappointments] = useState<getallAppoinmentForAdmin[]>(
    []
  );
        ...
        <Table className="w-full border rounded-lg shadow-sm">
          <TableHeader>
            <TableRow className="bg-gray-100">
              ...
              <TableHead className="px-4 py-2">Fee</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {appointments.map((appointment, index) => (
              <TableRow key={appointment.id}>
                ...
                <TableCell className="px-4 py-2">
                  <p
                    className={`${
                      (appointment.status === "pending" && "text-[#5E6DFF]") ||
