...
const Updatedoctor = () => {
  ...
  const [getpicture, setGetPicture] = React.useState("");
  ...
  React.useEffect(() => {
    getSingleDoctor(params?.id).then((res) => {
      if (res) {
  ...
  const onSubmit: SubmitHandler<addDoctors> = (data: addDoctors) => {
    ...
    updatedoctor(doctorData, params?.id).then(
      (
  ...
  return (
    <div>
      <section className="container mx-auto py-6 px-6 max-w-4xl">
        <h2 className="text-2xl font-semibold mb-4">Update Doctor Profile</h2>
              ...
              <div className="col-span-2 flex items-center gap-4">
                <div className="w-16 h-16 bg-gray-200 rounded-full flex items-center justify-center">
                  {image ? (
                    ...
                    <img
                      src={`${process.env.NEXT_PUBLIC_SUPABASE_IMAGE_URL}${getpicture}`}
                      alt=""
                    />
                  )}
                </div>
